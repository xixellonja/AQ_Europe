...
data_unval = pd.read_csv("/home/stud/kellezi/data/processed_unval_data.csv")
...
data_unval_ = scaler.fit_transform(data_unval)
data_unval_ = pd.DataFrame(data_unval_, columns=data_unval.columns)
...
X_train, X_test, y_train, y_test = train_test_split(
    ...
    #stratify = y # makes sure the same class label distribution is same as in the OG dataset. check how it effects learning curve )
...
plt.grid(True)
plt.legend()
plt.savefig('valscore_random42_.png', bbox_inches='tight')
plt.show()
...
plt.grid(True)
plt.legend()
#plt.savefig('Baseline Accuracy Score_random42.png', bbox_inches='tight')
plt.show()
...
X_unval = data_unval_.drop('PM2.5_Label', axis=1)
y_unval_true = data_unval_['PM2.5_Label']
#  MISSING_LABEL for np.nan on target var
MISSING_LABEL = np.nan 
y = np.full(y_true_badge.shape, MISSING_LABEL, dtype=float)
y_unval = np.full(y_unval_true.shape, MISSING_LABEL, dtype=float)
...
while len(labeled_indices(y)) < X_target:
    ...
    if len(unlabeled_idx) == 0:
        ...
        true_all, count_all = np.unique(y, return_counts=True)
        ...
        print("All data points have been labeled.", query)

        queried_batch = data_unval.iloc[query]

        #queried_countries = queried_batch["Country"].value_counts()
        #queried_labels = queried_batch["PM2.5_Label"].value_counts()
        queried_aq_type = queried_batch["Air Quality Station Type_Industrial"].value_counts()
        queried_aq_bg = queried_batch["Air Quality Station Type_Background"].value_counts()
        queried_aq_trf = queried_batch["Air Quality Station Type_Traffic"].value_counts()

        queried_ur = queried_batch["Air Quality Station Area_Urban"].value_counts()
        queried_sub = queried_batch["Air Quality Station Area_Suburban"].value_counts()
        queried_ru = queried_batch["Air Quality Station Area_Rural"].value_counts()
        
        #combi = queried_batch[['Air Quality Station Area_Urban', 'Air Quality Station Type_Background']].value_counts()
        #combi_less = queried_batch[['Air Quality Station Area_Rural', 'Air Quality Station Type_Industrial']].value_counts()

        #true labels
        labels = y[query]
        queried_labels, counts = np.unique(labels, return_counts=True)

        urb_back = queried_batch[['Air Quality Station Area_Urban', 'Air Quality Station Type_Background']].value_counts()
        rur_ind = queried_batch[['Air Quality Station Area_Rural', 'Air Quality Station Type_Industrial']].value_counts()
        urb_ind = queried_batch[['Air Quality Station Area_Urban', 'Air Quality Station Type_Industrial']].value_counts()
        rur_back = queried_batch[['Air Quality Station Area_Rural', 'Air Quality Station Type_Background']].value_counts()





        #queried_aq_area = queried_batch["Air Quality Station Area"].value_counts()
        print('true labels all', true_all, count_all)
        print(queried_labels, counts)
        print(queried_aq_type)
        print(queried_aq_bg)
        print(queried_aq_trf)
        print(queried_ur)
        print(queried_sub)
        print(queried_ru)
        print(urb_back, rur_ind, urb_ind, rur_back)
...
plt.title('MLP Classifier Performance (B=100)(seed=42)', fontsize=12)
plt.xlabel("Number of Labeled Instances", fontsize=12)
plt.ylabel("Accuracy", fontsize=12)
plt.grid()
plt.legend()
#plt.savefig('42AL Learning Accuracy Score__Badge_100_42.png', bbox_inches='tight')
plt.show()
...
ax.legend()
plt.savefig("42_constrained_clusters_c5.png", bbox_inches="tight")
plt.show()
...
ax.legend()
plt.savefig("42_constrained_clusters_c3.png", bbox_inches="tight")
plt.show()
...
ax.legend()
plt.savefig("42_constrained_clusters_c7.png", bbox_inches="tight")
plt.show()
...
plt.title('MLP Classifier Performance (query_size=100)(seed=42)', fontsize=12)
plt.xlabel("Number of Labeled Instances", fontsize=12)
plt.ylabel("Accuracy", fontsize=12)
plt.grid()
plt.legend()
plt.savefig('ALL_100_42_.png', bbox_inches='tight')
plt.show()
...
with open("label_acc_42.txt", "a") as file:
    file.write("Baseline_42: " + str(label_accuracies_baseline) + "\n")
    file.write("Badge_42: " + str(label_accuracies_badge) + "\n")
    file.write("TC7_42: " + str(label_accuracies_tc_7) + "\n")
    file.write("TC5_42: " + str(label_accuracies_tc_5) + "\n")
    file.write("TC3_42: " + str(label_accuracies_tc_3) + "\n")

    file.write("FULL_BL_42: " + str(accuracy_full) + "\n")
    file.write("FULL_BADGE_42: " + str(acc_badge) + "\n")
    file.write("FULL_TC7_42: " + str(acc_tc7) + "\n")
    file.write("FULL_TC5_42: " + str(acc_tc5) + "\n")
    file.write("FULL_TC3_42: " + str(acc_tc3) + "\n")
